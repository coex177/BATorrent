...
Большинство торрент-клиентов линкуют стандартный libtorrent. BATorrent поставляется с небольшим **пропатченным форком**, что позволяет менять поведение движка, недоступное через публичный API:

- **Быстрее разгон конвейера запросов.** На канале с высокой пропускной способностью и высокой задержкой стандартный конвейер запросов растёт по одному шагу; форк наращивает его геометрически и заполняет широкий канал за долю обменов. В собственном A/B-бенчмарке проекта — около **+27%** на быстром канале, без характерных для стандарта провалов между прогонами, и он никогда не деградирует.
- **Приоритет пиров из своей страны.** Офлайн-база GeoIP (db-ip Lite) помечает каждый пир по стране, и ранжирование пиров в форке при возможности предпочитает пиров из вашей страны — это обычно означает меньшую задержку и меньше троттлящихся трансграничных маршрутов.

Обе функции — возможности форка времени компиляции (выключены в стандартной сборке); они применяются как версионированные патчи в [`third_party/patches/`](third_party/patches), а не как встроенная копия.
